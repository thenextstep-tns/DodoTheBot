...
statuses = ["piano! Try typing dodo wisdom in the chat!", "it safe! Have you tried dodo future command?", "the Dodo card! Try catching a cat using dodo cat and summoning them with dodo summon (name of the cat here)!", "the goat. Steal a sweetroll from other players by pressing an emoji!", "gooseberry. Have you tried dodo parse? How about dodo pp?" ,"a big part in society. If you type 'schedule' or 'trials' i can send you our current schedule in DMs!", "it cool. Spread more love by typing dodo love (name of the user) in the chat!", "the system. If you wanna check how many cats you have, you can use dodo showcats command!", "the fool. If you want to catch a dog, spawn one with dodo dog" ,"with itself. Try dodo roll!", "ESO. If you wanna see the best or the worst parses, try dodo topparses/dodo bottomparses!", "dead", "on Fox's nerves", "dumb", "with fire"]
...
BAN_EMOJI = "🔨"  # The emoji to click to ban
SALUTE_EMOJI = "🫡"
TIMEOUT_DURATION = datetime.timedelta(days=1)
WAIT_DURATION = 8 * 60 * 60  # 8 hours in seconds

# -------------------------
# Relationship System Configurations
# -------------------------
MOUSE_ADOPTION_RANK = 250           # Threshold relationship points needed for adoption
RELATIONSHIP_BASE_POINTS = 20       # Base points used in the relationship calculation formula
CHEESE_DROP_THRESHOLD = 990         # In a 0-1000 random roll, drop cheese if roll > this value (~10% chance)
STAR_INSPIRATION_BOOST = 5               # Extra steps added when starry eyes event is triggered
STAR_INSPIRATION_DURATION = 5            # Number of moves during which the starry boost is active

# -------------------------
# Embed Color Settings (if not already defined)
# -------------------------
main_color = 0x3498db
success = 0x2ecc71
error = 0xe74c3c
...
trial_ping_roles = {
    ...
    # Add more trials and their role IDs as needed
}

# Dictionary of trial abbreviations used in channel names
trial_abbreviations = {
    "Normal Trials": 'Normal Trial',
    "Aetherian Archive": "vAA",  # Replace with actual role IDs (integers)
    "Hel Ra Citadel": "vHRC",
    "Sanctum Ophidia": "vSO",
    "Maw of Lorkhaj": "vMoL",
...
# Dictionary of trial images (URLs)


# --------------------------------------
# Clear Roles and User Ranks
# --------------------------------------

# Dictionary of veteran clear roles
vet_clears = {
    "Aetherian Archive": 819568084376485888,  # Replace with actual role IDs (integers)
    ...
    # Add more trials and their veteran clear role IDs as needed
}
...
# Dictionary of hard mode partial clear roles
...
# Dictionary of hard mode clear roles
hm_clears = {
    "Aetherian Archive": 861297811592839188,  # Replace with actual role IDs (integers)
    "Hel Ra Citadel": 861298098344558603,
    "Sanctum Ophidia": 861298387802783754,
    "Maw of Lorkhaj": 861301036127223868,
    "Halls of Fabrication": 861301105605738497,
    "Asylum Sanctorium": 861300693477359656,
    "Cloudrest": 861603911281541120,
    "Sunspire": 861603298176139294,
    "Kyne's Aegis": 861605027672227881,
    "Rockgrove": 861605943002005504,
    "Dreadsail Reef": 967757642061733929,
    "Sanity's Edge": 1115516504151183392,
    "Lucent Citadel": 1247554459127779379,
}
...
# (Optional) Dictionary mapping ranks to role IDs
rank_roles = {
...
# ANTI GRIEFING
# Time window in minutes to track pings
anti_griefing_time = 10
# Number of pings allowed before blacklisting
anti_griefing_limit = 5

OPENAI_API = "REDACTED_OPENAI_KEY"
PROXY_API = "REDACTED_API_KEY"

# MongoDB Connection
MONGO_URI = "mongodb+srv://dummyadmin:REDACTED_DB_PASSWORD@dummies.k0i3i.mongodb.net/dummies?retryWrites=true&w=majority"
CONNECTION_STRING = "mongodb+srv://dummyadmin:REDACTED_DB_PASSWORD@dummies.k0i3i.mongodb.net/dummies?retryWrites=true&w=majority"
client = MongoClient(CONNECTION_STRING)
db = client["DummiesBotDB"]
...
favor_col = db["Favor_tokens"] # This collection is declared but not used in the current cog.
...
player_roles = { u"\U0001F6E1" : 860466479400353853 , u"\u2695" : 860466414237384714, u"\u2694" : 860466132380549130}
base_roles_msg = 967767923794006026

public_channels = [783594414085111840, 986220407210262528, 1081595613340258314, 1192144724920893480, 1020227637513306142, 1020222103473115216, 1020221746995011615, 1020220157878075433, 1081456941232050216, 799654786537881641,
783631231392415774, 1081450115052605461]

#new role message
reaction_roles = {
    1238462248167673889: {  # Trial Roles
        u"\U0001F9D9": 819566620183494706,  # Mage - Aetherian Archive
        u"\U0001F4A3": 819566756992909353,  # Bomb - Hel Ra Citadel
        u"\U0001F40D": 819566868649082891,  # Snake - Sanctum Ophidia
        u"\U0001F640": 819566954943479839,  # Screamo Cat - Maw of Lorkhaj
        u"\U0001F916": 819568455056621610,  # Robot - Halls of Fabrication
        u"\U0001F9BF": 819567118710079509,  # Mech Leg - Asylum Sanctorium
        u"\U0001F40C": 829297389280952320,  # Snail - Cloudrest
        u"\U0001F432": 819568232121761832,  # Dragon - Sunspire
        u"\U0001F9DB": 819568879906324490,  # Vampire - Kyne's Aegis
        u"\U0001F525": 844105629445521429,  # Fire - Rockgrove
        u"\U00002693": 967759030414086174,  # Anchor - Dreadsail Reef
        u"\U0001F4A4": 1094675660158533744,  # ZZZ - Sanity's Edge
        u"\U0001F3F0": 1238462328304046131,  # Castle - Lucent Citadel
        u"\U000026D3": 1362424126249111562,  # Chains - Ossein Cage

    },
    
    1238466517969141831: {  # Other content notifications
        u"\U0001F476": 846300383982125088,  # Baby - Normal Trials
        u"\U0001F4AA": 844105870392819723,  # Bicep - Challenger Team
        u"\U0001F393": 844160810813358100,  # Grad Hat - Trial Training
        u"\U000023F0": 1238530302805020793, # Timer - ASAP trials
        u"\U0001F985": 944514526055915520,  # Eagle - AD PvP
        u"\U0001F981": 944515219701530634,  # Lion's head - DC PvP
        u"\U0001F409": 944515630642638888,  # Dragon (full) - EP PvP
        u"\U00002694": 844473322170351637,  # Crossed Swords - PvP
        u"\U0001F419": 1246324926521020457, # Octopus - infinite archive
    },
    
    1238534410240921713: {  # Social roles
        u"\U0001F46A": 807064226697969686,  # Family - In-game guild
        u"\U0001F973": 801802675678085121,  # Partying face - Party Parrot
        u"\U0001F3E1": 844099301021712384,  # House with garden - Houseling
        u"\U0001FA9A": 844472691372064788,  # Saw - Crafter
        u"\U00002753": 967760954215530496,  # Question mark - Trivia Enjoyers
    },
...
allowed_links = ["discord.gg/8ewt2Fe", "discord.gg/esou",
                "discord.gg/uesp", "discord.gg/ZaPwNHKQBg",
...
roll_limit = 50000000
roll_challenge_emoji = { u"\U0001F3B2" : 0, u"\u26D4" : 1}
roll_duel_emoji = { u"\U0001F3B2" : 0}
dice_gifs = ["https://tenor.com/tXuH.gif", "https://tenor.com/bmrvd.gif", "https://tenor.com/bkjAq.gif",
...
CHALLENGES = ["post an embarrassing childhood photo",
...
valentinegifs = ['https://tenor.com/bRGK9.gif', 'https://tenor.com/bUem4.gif', 'https://tenor.com/bjloR.gif',
...
kittengifs = ['https://tenor.com/YMKv.gif', "https://tenor.com/bTCjb.gif", "https://tenor.com/bQ9Vs.gif",
...
#waifu
waifuquotes = ["The world is not beautiful. And that, in a way, lends it a sort of beauty",
...
# SELF-CARE
selfCareOptions = [["Take a short nap when you can", 50, 20, 60],
...
#Addons
addons = { u"\U0001F440" : 0 }

#Pet config : Class name, StrMin, StrMax, AgiMin, AgiMax, IntMin, IntMax, CharmMin, CharmMax

pet_emoji = { u"\U0001F431" : 0, u"\U0001F436" : 1}
versus = "versus.png"

catclasses = [["Chonk", 30, 50, 1, 7, 2, 10, 5, 15], ["Zoomies Pro", 5, 15, 30, 50, 1, 7, 2, 10], ["Smarty Pants", 2, 10, 5, 15, 30, 50, 1, 7], ["Aww summoner", 1, 7, 2, 10, 5, 15, 30, 50]]
dogclasses = [["Working Dog", 40, 50, 1, 5, 3, 8, 20, 30], ["Hound Dog", 20, 30, 40, 50, 1, 5, 3, 8], ["Herding Dog", 3, 8, 20, 30, 40, 50, 1, 5], ["Companion Dog", 1, 5, 3, 8, 20, 30, 40, 50]]

pet_actions = { u"\U0001F41F" : "Toggle Fishing", u"\U0001F4AA" : "Send the cat to the gym"	}
fishing_result_actions = { u"\U0001F392" : "Put in the goodies bag", u"\U0001F4B0" : "Sell", u"\U0001F4A5" : "Throw away"}
fishing_cost = 10

gym_cost = 50
fighting_cost = 100
fight_actions = { u"\u270A" : 0, u"\U0001F4A8" : 1, u"\U0001F6E1" : 2, u"\U0001F60D" : 3 }

item_qualities = { u"\U0001F7E2" : "Broken", u"\U0001F535": "Affordable", u"\U0001F7E3" : "Superb", u"\U0001F7E1" : "Perfect", u"\U0001F7E0" : "Unique" }
item_types = { u"\U0001F5D1" : "Trash", u"\U0001F6E1" : "Armor", u"\U0001F354" : "Buff food", u"\U0001F48E" : "Treasure" }

tenor_api = "REDACTED_TENOR_KEY"
