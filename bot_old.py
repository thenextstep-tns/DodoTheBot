...
import discord, asyncio, os, platform, sys, pymongo, dns, random
...
async def on_ready():
	...
	#print(f"Database {db} with the collection {collection} are connected")
	print("-------------------")
...
async def status_task():
	while True:
		await bot.change_presence(activity=discord.Game("possum"))
		await asyncio.sleep(3600)
		await bot.change_presence(activity=discord.Game("Dodos support"))
		await asyncio.sleep(3600)
		await bot.change_presence(activity=discord.Game("dodo"))
		await asyncio.sleep(3600)
		await bot.change_presence(activity=discord.Game("with Keegan's pp"))
		await asyncio.sleep(180)
		await bot.change_presence(activity=discord.Game("piano"))
		await asyncio.sleep(3600)
...
async def on_member_join(member):
    channel = bot.get_channel(config.WAYSHRINE)
    userid = member.id
    print (userid)
    await channel.send("Welcome, <@" + str(userid) + ">! If you want to raid with us, post your clearsies in <#" + str(config.RANK_REQ) + "> and choose the trials you would like to join in <#" + str(config.SELECT_ROLES) + ">. Ping any of the admins if you have any questions, and happy raiding! :hearts: ")
    #role = discord.utils.get(member.guild.roles, name="Dummies")
...
async def on_message(message):
	...
	elif message.author == bot.user or message.author.bot:
	    return
	# If that was a user, we see it in the terminal
	else:
	    ...
	    elif("master" in msg.lower()):
	        #399644269973078026
	        masterseed = random.randint(0, 100)
	        if masterseed == 100:
	            channel = bot.get_channel(message.channel.id)
	            await channel.send("Oh MASTER'S RESTORATION STAFF IS ACTUALLY REALLY USEFUL")
	            await asyncio.sleep(random.randint(1,5))
	            await channel.send("It's a very good set that drops in dragonstar arena")
	            await asyncio.sleep(random.randint(2,4))
	            await channel.send("People often say nDSA or vDSA instead")
	            await asyncio.sleep(random.randint(3,4))
	            await channel.send("It depends on the difficulty level")
	            await asyncio.sleep(random.randint(1,2))
	            await channel.send("n stands for NORMAL, v stands for VETERAN")
	            await asyncio.sleep(random.randint(3,4))
	            await channel.send("or veterinarian or vagina in very specific cases")
	            await asyncio.sleep(random.randint(2,5))
	            await channel.send("so 'nDSA' means normal dragonstar arena")
	            await asyncio.sleep(random.randint(1,3))
	            await channel.send("and 'vDSA' means... guess what?")
	            await asyncio.sleep(random.randint(1,5))
	            await channel.send("very useful set you absolutely should get it")
	            await asyncio.sleep(random.randint(2,5))
	            await channel.send("i mean staff, not the set")
	            await asyncio.sleep(random.randint(0,3))
	            await channel.send(":joy:")
	            await asyncio.sleep(random.randint(2,5))
	            await channel.send("but if you get it in NIRNHONED u better decon it")
	            await asyncio.sleep(random.randint(2,5))
	            await channel.send("or Pryia will kick your ass")
	            await asyncio.sleep(random.randint(2,5))
	            
	            #await bot.send_message(message.channel, f"Did someone just say Ducky? Is he late for a raid again? {user.mention} {user.mention}, come come come]")
	            #await channel.send("Did someone just say Ducky? Is he late for a raid again?? @Ducky#9769 @Ducky#9769 @Ducky#9769 come come come")
	        else:
	            print ("We detected a master but the seed wasn't large enough")
	    
	    
	    elif("dumb dodo" in msg.lower() or "stoopid motherfucking bird" in msg.lower()  or "stoopid dodo" in msg.lower()  or "stupid dodo" in msg.lower()  or "stoopid bird" in msg.lower()  or "stupid bird" in msg.lower()  or "fkn dodo" in msg.lower() or "fk u dodo" in msg.lower() or "fk u dodo" in msg.lower() or "dodo fk you" in msg.lower()or "fk u dodo" in msg.lower()or "dodo u suck" in msg.lower() or "u suck, dodo" in msg.lower() or "you suck dodo" in msg.lower() or "u suck dodo" in msg.lower() or "dodo fuck off" in msg.lower() or "dodo, fuck off" in msg.lower() or "piss off dodo" in msg.lower() or "fuck off, dodo" in msg.lower() or "fuck off dodo" in msg.lower() or "dodo is a bit dumb" in msg.lower() or "dodo is an idiot" in msg.lower() or "dodo is dumb" in msg.lower() or "dodo is stupid" in msg.lower() or "fuck you, dodo" in msg.lower() or "f u dodo" in msg.lower() or "dodo fuck you" in msg.lower() or "fuck u dodo" in msg.lower() or "fuck you dodo" in msg.lower() or "dodo bad" in msg.lower() or "bad dodo" in msg.lower() or "dodo dumb" in msg.lower() or "stupid dodo" in msg.lower() or "dodo stupid" in msg.lower() or "idiot dodo" in msg.lower() or "dodo idiot" in msg.lower() or "retarded dodo" in msg.lower() or "dodo retarded" in msg.lower()):
	        channel = bot.get_channel(message.channel.id)
...
#        if "shoosh" in str(ctx.content.lower()):
...
#            updatedPost = {"You shooshed Dodo ", score, " times"}
#            await ctx.channel.send(updatedPost)
    

# The code in this event is executed every time a command has been *successfully* executed

#@bot.event
#async def on_raw_reaction_add(reaction, user : discord.Member = None):
#    if not user:
#        user = reaction.user_id
#    channel = reaction.channel_id
#    emoji = reaction.emoji
#    print ("We detected a reaction in a channel with the id " + str(channel) + " from user " + str(user) + " to message with id " + str(reaction.message_id))
#    if reaction.message_id == "865666538065362956":
#        print ("Bingo!")
    #user = await self.bot.fetch_user(reaction.user_id)
    #if reaction.message.channel.id != 844097897321463868:
    #    return
    #if reaction.emoji == "🥰":
    #  Role = discord.utils.get(user.server.roles, name="happy")
...
async def on_command_completion(ctx):
	...
	channel = bot.get_channel(855701669168807936)
	await channel.send(f"Executed {executedCommand} command in {ctx.guild.name} (ID: {ctx.message.guild.id}) by {ctx.message.author} (ID: {ctx.message.author.id})")
