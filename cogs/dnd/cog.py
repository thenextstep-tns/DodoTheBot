...
import lang
from cogs.dnd import context, embeds
...
class Tabletop(commands.Cog, name="dnd"):
    ...
    async def campaign_create(
        ...
        if interaction.guild_id is None:
            await interaction.response.send_message(lang.TT_NEEDS_GUILD, ephemeral=True)
            return

        name = name.strip()[:MAX_NAME]
        if not name:
            await interaction.response.send_message(lang.TT_CAMPAIGN_NEEDS_NAME, ephemeral=True)
            return

        repo = campaigns_for(interaction.guild_id)
        if repo.by_name(name):
            await interaction.response.send_message(
                lang.TT_CAMPAIGN_EXISTS.format(name=name), ephemeral=True
            )
            return

        key = ruleset or self.bot.params.get(interaction.guild_id, "dnd_default_ruleset")
        ...
        await interaction.response.send_message(
            lang.TT_CAMPAIGN_CREATED.format(name=name, ruleset=resolved.label)
        )
    ...
    async def campaign_join(
        ...
        if found.campaign.is_member(interaction.user.id):
            await interaction.response.send_message(
                lang.TT_CAMPAIGN_ALREADY_IN.format(name=found.campaign.name), ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            lang.TT_CAMPAIGN_JOINED.format(name=found.campaign.name), ephemeral=True
        )
    ...
    async def campaign_leave(
        ...
        if not campaign.is_member(interaction.user.id):
            await interaction.response.send_message(
                lang.TT_CAMPAIGN_NOT_IN.format(name=campaign.name), ephemeral=True
            )
            return
        ...
        if campaign.is_gm(interaction.user.id) and len(campaign.gm_ids) == 1:
            await interaction.response.send_message(
                lang.TT_GM_CANNOT_LEAVE.format(name=campaign.name), ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            lang.TT_CAMPAIGN_LEFT.format(name=campaign.name), ephemeral=True
        )
    ...
    async def character_create(
        ...
        if existing is not None:
            await interaction.response.send_message(
                lang.TT_CHARACTER_EXISTS.format(name=existing.name, campaign=found.campaign.name),
                ephemeral=True,
            )
            return
        ...
        await interaction.response.send_message(
            lang.TT_CHARACTER_CREATED.format(name=entity.name, campaign=found.campaign.name),
    ...
    async def character_sheet(
        ...
        if entity is None:
            await interaction.response.send_message(
                lang.TT_NO_CHARACTER.format(campaign=found.campaign.name), ephemeral=True
            )
            return
    ...
    async def character_retire(
        ...
        await interaction.response.send_message(
            lang.TT_CHARACTER_RETIRED.format(name=entity.name)
        )
    ...
    async def scene_open(
        ...
        if found.store.scenes.open_in_channel(interaction.channel_id) is not None:
            await interaction.response.send_message(lang.TT_SCENE_EXISTS, ephemeral=True)
            return
        ...
        await interaction.response.send_message(
            lang.TT_SCENE_OPENED.format(title=opened.title),
    ...
    async def scene_close(self, interaction: discord.Interaction) -> None:
        ...
        if open_scene is None:
            await interaction.response.send_message(lang.TT_SCENE_NONE, ephemeral=True)
            return
        ...
        await interaction.response.send_message(
            lang.TT_SCENE_CLOSED.format(title=open_scene.title)
        )
    ...
    async def dice_roll(
        ...
        max_sides = self.bot.params.get(interaction.guild_id, "dnd_max_sides")
        try:
            spec = dice.parse(expression, max_dice=max_dice, max_sides=max_sides)
        except dice.DiceLimitError as limit:
            await interaction.response.send_message(
                lang.TT_ROLL_TOO_BIG.format(max_dice=limit.max_dice, max_sides=limit.max_sides),
                ephemeral=True,
            )
            return
        if spec is None:
            await interaction.response.send_message(
                lang.TT_ROLL_INVALID.format(expr=expression), ephemeral=True
            )
            return
    ...
    async def dnd_migrate(
        ...
        if context.guild is None:
            await context.send(lang.TT_MIGRATE_NO_GUILD)
            return
        ...
        body = lang.TT_MIGRATE_HEADER.format(
